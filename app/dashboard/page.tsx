...
export default function DashboardPage() {
  ...
  const statCards = [
    { label: 'Learning Streak', value: `${currentUser.streak} Days`, icon: Flame, badge: '+1 today', color: 'text-brand-orange', bg: 'bg-brand-orange/10' },
    { label: 'Modules Completed', value: `${currentUser.modules_completed} / 48`, icon: BookOpen, color: 'text-blue-500', bg: 'bg-blue-50' },
    { label: 'Overall Progress', value: `${currentUser.overall_progress}%`, icon: BarChart2, color: 'text-purple-500', bg: 'bg-purple-50' },
    { label: 'Next Deadline', value: 'In 3 Days', icon: Clock, color: 'text-brand-danger', bg: 'bg-red-50' },
  ]
      ...
      <div className="max-w-[1400px] mx-auto px-6 py-6">
        ...
        <section className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-7">
          {loading
            ...
            : statCards.map(({ label, value, icon: Icon, badge, color, bg }, i) => (
                <motion.div
                  key={label}
                ...
                >
                  <div className="flex justify-between items-start">
                    ...
                    {badge && (
                      <span className="text-xs font-semibold text-brand-success bg-green-50 px-2 py-0.5 rounded-full border border-green-100">
                        {badge}
                      </span>
                    )}
                  </div>
        ...
        <motion.section {...FADE_UP} transition={{ delay: 0.15, duration: 0.35 }} className="mb-7">
          <div className="bg-white rounded-2xl border border-brand-border shadow-card overflow-hidden flex flex-col md:flex-row">
            <div className="w-full md:w-72 h-44 md:h-auto flex-shrink-0 relative">
              <img
                src={enrolledCourse.thumbnail_url}
                alt={enrolledCourse.title}
                className="w-full h-full object-cover"
              />
              <div className="absolute inset-0 bg-gradient-to-r from-black/20 to-transparent" />
              <span className="absolute top-3 left-3 text-[10px] font-bold bg-brand-orange text-white px-2.5 py-1 rounded-full">
                IN PROGRESS
              </span>
            </div>
            <div className="flex-1 p-6 flex flex-col justify-between">
              <div>
                <h3 className="font-sora text-xl font-bold text-brand-navy">{enrolledCourse.title}</h3>
                <p className="text-brand-muted text-sm font-dm-sans mt-1">
                  {enrolledCourse.modules.find((m) => m.current)?.title ?? 'Module 4: Exploratory Data Analysis'}
                </p>

                <div className="mt-5">
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-sm font-semibold text-brand-navy font-dm-sans">{enrolledCourse.progress}% Complete</span>
                    <span className="text-xs text-brand-muted font-dm-sans">Step 4 of {enrolledCourse.total_modules}</span>
                  </div>
                  <div className="w-full bg-gray-100 rounded-full h-2 overflow-hidden">
                    <motion.div
                      className="bg-brand-orange h-full rounded-full"
                      initial={{ width: 0 }}
                      animate={{ width: `${enrolledCourse.progress}%` }}
              ...
              <div className="flex items-center gap-4 mt-5">
                <Link
                  ...
                  className="flex items-center gap-2 bg-brand-orange text-white px-5 py-2.5 rounded-xl font-semibold text-sm font-dm-sans
                             hover:bg-brand-orange-dark transition-all active:scale-[0.98]"
                >
                  <Play size={15} className="fill-white" /> Resume Learning
                </Link>
                <Link href="/courses" className="text-brand-navy text-sm font-semibold font-dm-sans hover:text-brand-orange transition">
                  Course Details →
                </Link>
              </div>
            </div>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-12 gap-5 mb-7">
          ...
          <motion.div
            ...
            className="lg:col-span-7 bg-white rounded-2xl border border-brand-border shadow-card p-6"
          >
            <div className="flex justify-between items-center mb-5">
              <h3 className="font-sora text-lg font-bold text-brand-navy">Your Learning Path</h3>
              <Link href="/courses" className="text-brand-orange text-sm font-semibold font-dm-sans hover:underline">
                View All →
              </Link>
            </div>
            <div className="relative">
              ...
              <div className="space-y-5">
                {enrolledCourse.modules.map((module) => (
                  <div key={module.id} className="relative pl-10 flex items-start gap-3">
                    <div className={clsx(
                      ...
                      {module.completed ? <CheckCircle size={14} className="text-white fill-white" />
                        : module.current ? <Play size={12} className="text-white fill-white" />
                        : <Lock size={12} className="text-gray-400" />}
                    </div>
                    <div className="flex-1">
                      <h4 className={clsx(
                        'text-sm font-semibold font-dm-sans',
                        module.current ? 'text-brand-orange' : module.completed ? 'text-brand-navy' : 'text-gray-400',
                      )}>
                        {module.title}
                      </h4>
                      <p className={clsx('text-xs font-dm-sans mt-0.5', module.locked ? 'text-gray-300' : 'text-brand-muted')}>
                        {module.completed ? `Completed ✓` : module.current ? '45% complete · ~2h left' : module.locked ? 'Locked — complete previous module' : `${module.duration} min`}
                      </p>
                    </div>
          ...
          <motion.div
            ...
            className="lg:col-span-5 bg-white rounded-2xl border-2 border-brand-orange/20 shadow-card p-6 flex flex-col gap-4 relative overflow-hidden"
          >
            ...
            <div className="space-y-3">
              {insights.map((ins) => (
                <div
                  key={ins.id}
                  className={clsx(
                    'p-3.5 rounded-xl text-sm font-dm-sans border-l-4',
                    ins.type === 'recommendation' ? 'bg-brand-orange/5 border-brand-orange' :
                    ins.type === 'warning' ? 'bg-yellow-50 border-brand-warning' : 'bg-green-50 border-brand-success',
                  )}
                >
                  <p className={clsx(
            ...
            <Link
              href="/ai-tutor"
              className="w-full flex items-center justify-center gap-2 border-2 border-brand-orange text-brand-orange font-semibold text-sm font-dm-sans
                         py-2.5 rounded-xl hover:bg-brand-orange hover:text-white transition-all"
            >
          ...
          <div className="flex justify-between items-center mb-4">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="w-9 h-9 rounded-full bg-white border border-brand-border flex items-center justify-center hover:border-brand-orange/40 transition"
              >
                <ChevronLeft size={16} className="text-gray-600" />
              </button>
              <button
                onClick={() => setSessionIdx((i) => Math.min(liveSessions.length - 1, i + 1))}
                className="w-9 h-9 rounded-full bg-white border border-brand-border flex items-center justify-center hover:border-brand-orange/40 transition"
              >
              ...
              >
                <div className="flex justify-between items-start">
                  <span className={clsx(
                    ...
                    session.is_today ? 'bg-brand-orange/10 text-brand-orange' : 'bg-gray-100 text-gray-500',
                  )}>
                    {session.date}
                  </span>
                  <div className="flex items-center gap-1 text-xs text-brand-muted font-dm-sans">
                    <Users size={12} />
                    <span>+{session.participants}</span>
                  </div>
                </div>
                <div>
                  <h4 className="font-sora text-sm font-bold text-brand-navy group-hover:text-brand-orange transition">
                    {session.title}
                  </h4>
                ...
                <button
                  ...
                  className={clsx(
                    'w-full py-2 rounded-lg text-xs font-semibold font-dm-sans transition-colors flex items-center justify-center gap-1.5',
                    session.is_today
                      ? 'bg-brand-navy text-white hover:bg-brand-orange'
                      : 'border border-brand-border text-gray-600 hover:border-brand-orange/40 hover:text-brand-orange',
                  )}
                >
