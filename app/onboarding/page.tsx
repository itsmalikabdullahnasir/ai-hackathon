  ...
  return (
    <button onClick={onClick}
      className={clsx(
        ...
        selected ? 'border-brand-orange bg-brand-orange/5 text-brand-orange' : 'border-brand-border bg-white text-gray-700 hover:border-brand-orange/40 hover:bg-brand-orange/3',
      )}
    >
      <div className={clsx('w-5 h-5 rounded-full border-2 flex-shrink-0 flex items-center justify-center transition-all', selected ? 'border-brand-orange bg-brand-orange' : 'border-gray-300')}>
...
export default function OnboardingPage() {
  ...
  useEffect(() => {
    // If already logged in and onboarding done, redirect to dashboard
  ...
  async function handleFinish() {
    ...
    // Check if user is logged in; if not, create a guest session via magic link or just proceed
    const { data: sessionData } = await supabase.auth.getSession()

    if (!sessionData.session) {
      // User came from "Get started" without logging in — redirect to login
      showToast('Please sign in to save your learning path.', 'info')
      router.push('/login')
      return
    }

    try {
      await apiFetch('/api/onboarding-assess', {
        method: 'POST',
        body: JSON.stringify({
          course_id: result.course_id,
          level: result.level,
          skill_tags: result.skills,
          hours_per_week: data.hours,
          goal: data.goal,
        }),
      })
  ...
  return (
    <div className="min-h-screen bg-brand-navy flex flex-col items-center justify-center p-6">
      <div className="flex items-center gap-2.5 mb-8">
        ...
        <h1 className="font-sora text-white text-lg font-bold">auto<span className="text-brand-orange">camp</span></h1>
      </div>
      ...
      {step < 5 && (
        <div className="w-full max-w-lg mb-6">
          ...
          <div className="w-full bg-white/10 rounded-full h-1.5">
            <motion.div className="bg-brand-orange h-full rounded-full" animate={{ width: `${progress}%` }} transition={{ duration: 0.3 }} />
          </div>
        </div>
      )}
      ...
      <div className="w-full max-w-lg">
        <AnimatePresence mode="wait">
          <motion.div key={step} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.2 }} className="bg-white rounded-2xl shadow-xl p-8">
            ...
            {step === 1 && (
              <div className="space-y-5">
                ...
                <div className="space-y-2.5">
                  {backgrounds.map((b) => <OptionCard key={b} label={b} selected={data.background === b} onClick={() => setData((d) => ({ ...d, background: b }))} />)}
                </div>
                <button onClick={next} disabled={!data.background} className="w-full bg-brand-orange text-white py-3 rounded-xl font-semibold text-sm font-dm-sans hover:bg-brand-orange-dark transition disabled:opacity-40 flex items-center justify-center gap-2">
            ...
            {step === 2 && (
              <div className="space-y-5">
                ...
                <div className="space-y-2.5">
                  {pythonLevels.map((p) => <OptionCard key={p} label={p} selected={data.python === p} onClick={() => setData((d) => ({ ...d, python: p }))} />)}
                </div>
                <div className="flex gap-3">
                  <button onClick={back} className="flex items-center gap-1 text-brand-muted text-sm font-dm-sans hover:text-brand-navy transition"><ChevronLeft size={16} /> Back</button>
                  <button onClick={next} disabled={!data.python} className="flex-1 bg-brand-orange text-white py-3 rounded-xl font-semibold text-sm font-dm-sans hover:bg-brand-orange-dark transition disabled:opacity-40 flex items-center justify-center gap-2">Continue <ChevronRight size={16} /></button>
                </div>
              </div>
            )}
            ...
            {step === 3 && (
              <div className="space-y-5">
                ...
                <div className="space-y-2.5">
                  {goals.map((g) => <OptionCard key={g} label={g} selected={data.goal === g} onClick={() => setData((d) => ({ ...d, goal: g }))} />)}
                </div>
                <div className="flex gap-3">
                  <button onClick={back} className="flex items-center gap-1 text-brand-muted text-sm font-dm-sans hover:text-brand-navy transition"><ChevronLeft size={16} /> Back</button>
                  <button onClick={next} disabled={!data.goal} className="flex-1 bg-brand-orange text-white py-3 rounded-xl font-semibold text-sm font-dm-sans hover:bg-brand-orange-dark transition disabled:opacity-40 flex items-center justify-center gap-2">Continue <ChevronRight size={16} /></button>
                </div>
              </div>
            )}
            ...
            {step === 4 && (
              <div className="space-y-5">
                ...
                <div className="space-y-2.5">
                  {timeOptions.map((t) => <OptionCard key={t} label={t} selected={data.hours === t} onClick={() => setData((d) => ({ ...d, hours: t }))} />)}
                </div>
                <div className="flex gap-3">
                  <button onClick={back} className="flex items-center gap-1 text-brand-muted text-sm font-dm-sans hover:text-brand-navy transition"><ChevronLeft size={16} /> Back</button>
                  <button onClick={next} disabled={!data.hours} className="flex-1 bg-brand-orange text-white py-3 rounded-xl font-semibold text-sm font-dm-sans hover:bg-brand-orange-dark transition disabled:opacity-40 flex items-center justify-center gap-2">Build My Path <Sparkles size={16} /></button>
                </div>
              </div>
            )}
              ...
              <div className="space-y-6 text-center">
                ...
                <div className="bg-brand-bg rounded-xl border border-brand-border p-5 text-left space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span className={clsx('text-xs font-bold px-3 py-1 rounded-full border capitalize', levelColors[result.level])}>{result.level}</span>
                  </div>
                  <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {result.skills.map((s) => (
                      <span key={s} className="text-xs px-3 py-1 bg-brand-orange/10 text-brand-orange rounded-full font-dm-sans font-medium">{s}</span>
                    ))}
                  </div>
                </div>
                <button onClick={handleFinish} disabled={submitting}
                  className="w-full bg-brand-orange text-white py-3.5 rounded-xl font-semibold font-dm-sans text-sm hover:bg-brand-orange-dark transition-all flex items-center justify-center gap-2 shadow-sm disabled:opacity-60"
                >
                  {submitting ? <><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />Saving…</> : 'Start Learning → Dashboard'}
                </button>
              </div>
            )}
          </motion.div>
...
function LoadingStep({ onDone }: { onDone: () => void }) {
  const steps = ['Analyzing your background…', 'Mapping skill gaps…', 'Selecting optimal course…', 'Building your learning path…']
  ...
  return (
    <div className="text-center py-8 space-y-6">
      ...
      <div>
        ...
        <AnimatePresence mode="wait">
          <motion.p key={current} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="text-brand-muted text-sm font-dm-sans">
            {steps[current]}
          </motion.p>
        </AnimatePresence>
      </div>
      <div className="flex justify-center gap-1.5">
        {steps.map((_, i) => (
          <div key={i} className={clsx('w-2 h-2 rounded-full transition-all', i <= current ? 'bg-brand-orange' : 'bg-gray-200')} />
        ))}
      </div>
    </div>
  )
