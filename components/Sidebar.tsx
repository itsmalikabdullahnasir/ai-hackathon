    ...
    <aside className="fixed left-0 top-0 h-screen w-[260px] bg-brand-navy flex flex-col z-40 shadow-xl">
      <div className="px-6 py-6 border-b border-white/5">
        <Link href="/dashboard" className="flex items-center gap-2.5">
          ...
          <div>
            <h1 className="font-sora text-white text-lg font-bold leading-none">auto<span className="text-brand-orange">camp</span></h1>
          ...
          return (
            <Link key={href} href={href}
              className={clsx(
                ...
                active ? 'bg-brand-orange/10 text-brand-orange border-l-[3px] border-brand-orange' : 'text-white/50 hover:text-white hover:bg-white/5 border-l-[3px] border-transparent',
              )}
            >
      ...
      <div className="px-3 pb-6 border-t border-white/5 pt-3 space-y-2">
        <div className="flex items-center gap-3 px-4 py-3 rounded-lg bg-white/5">
          {avatarUrl ? (
            <img src={avatarUrl} alt={displayName} className="w-8 h-8 rounded-full object-cover border-2 border-brand-orange/40" />
          ) : (
            <div className="w-8 h-8 rounded-full border-2 border-brand-orange/40 bg-brand-orange/15 text-brand-orange text-[10px] font-semibold flex items-center justify-center">{initials}</div>
          )}
        ...
        <button onClick={handleLogout} className="w-full flex items-center gap-3 px-4 py-2.5 rounded-lg text-white/50 hover:text-red-400 hover:bg-red-500/10 transition-all text-sm font-dm-sans">
          <LogOut size={16} />
          <span>Log out</span>
        </button>
      </div>
