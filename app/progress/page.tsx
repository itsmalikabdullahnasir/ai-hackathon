...
export default function ProgressPage() {
  ...
  useEffect(() => {
    apiFetch<ProgressData>('/api/progress')
      .then((d) => setData(d))
      .catch(() => showToast('Failed to load progress data', 'error'))
  ...
  return (
    <AppShell>
      <div className="max-w-[1400px] mx-auto px-6 py-6">
        ...
        {/* Activity Heatmap */}
        <motion.section {...FADE_UP} transition={{ delay: 0.1 }} className="bg-white rounded-2xl border border-brand-border shadow-card p-6 mb-7 overflow-x-auto">
              ...
              {Array.from({ length: 52 }).map((_, w) => (
                <div key={w} className="flex flex-col gap-1">
                  {Array.from({ length: 7 }).map((_, d) => {
                    const cell = (data?.heatmap ?? []).find((c) => c.week === w && c.day === d)
        ...
        {/* Completed Modules + Leaderboard */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-7">
          <motion.div {...FADE_UP} transition={{ delay: 0.25 }} className="lg:col-span-2 bg-white rounded-2xl border border-brand-border shadow-card overflow-hidden">
            <div className="px-6 py-4 border-b border-brand-border flex justify-between items-center">
              ...
              <span className="text-xs text-brand-muted font-dm-sans">{data?.completedModules?.length ?? 0} modules</span>
            </div>
            {loading ? (
                  ...
                  <tbody>
                    {(data?.completedModules ?? []).map((m, i) => (
                      <tr key={i} className="border-b border-brand-border hover:bg-brand-bg/50 transition group">
                        ...
                        <td className="px-5 py-3.5">
                          <button onClick={() => showToast('Certificate download started!', 'success')} className="flex items-center gap-1.5 text-xs text-brand-orange font-semibold font-dm-sans opacity-0 group-hover:opacity-100 transition hover:underline">
          ...
          <motion.div {...FADE_UP} transition={{ delay: 0.3 }} className="bg-white rounded-2xl border border-brand-border shadow-card p-6">
            ...
            {loading ? (
              ...
              <p className="text-brand-muted text-sm font-dm-sans text-center py-8">Join a cohort to see rankings.</p>
            ) : (
              <div className="space-y-3">
                {(data?.leaderboard ?? []).map((entry) => (
                  <div key={entry.rank} className={clsx('flex items-center gap-3 p-3 rounded-xl transition', entry.is_me ? 'bg-brand-orange/5 border-2 border-brand-orange/20' : 'hover:bg-brand-bg')}>
                    <span className={clsx('text-lg w-6 text-center flex-shrink-0', entry.badge ? '' : 'text-brand-muted text-sm font-bold')}>{entry.badge || `#${entry.rank}`}</span>
                    <div className="flex-1">
                      <p className={clsx('text-sm font-semibold font-dm-sans', entry.is_me ? 'text-brand-orange' : 'text-brand-navy')}>
        ...
        {/* Certificates */}
        <motion.section {...FADE_UP} transition={{ delay: 0.35 }} className="bg-white rounded-2xl border border-brand-border shadow-card p-6">
          <div className="flex items-center gap-2 mb-5">
            <Award size={20} className="text-brand-orange" />
            <h3 className="font-sora text-lg font-bold text-brand-navy">Earned Certificates</h3>
          </div>
          {loading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">{Array.from({ length: 2 }).map((_, i) => <div key={i} className="h-20 bg-gray-100 rounded-xl animate-pulse" />)}</div>
          ) : (data?.certificates ?? []).length === 0 ? (
            <p className="text-brand-muted text-sm font-dm-sans text-center py-8">Complete a course to earn your first certificate!</p>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {(data?.certificates ?? []).map((cert) => (
                <div key={cert.id} className="flex items-center gap-4 p-4 border-2 border-brand-orange/20 rounded-xl bg-brand-orange/3 hover:bg-brand-orange/5 transition group">
                  <div className="w-12 h-12 bg-brand-orange/10 rounded-xl flex items-center justify-center flex-shrink-0"><Award size={24} className="text-brand-orange" /></div>
                  <div className="flex-1">
                  ...
                  <button onClick={() => showToast('Downloading certificate PDF…', 'success')} className="text-brand-orange opacity-0 group-hover:opacity-100 transition"><Download size={18} /></button>
                </div>
              ))}
            </div>
          )}
