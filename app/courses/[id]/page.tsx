...
import { courses, quizzes } from '@/lib/mockData'
...
const resources = [
  { id: 'r1', title: 'EDA Cheat Sheet (PDF)', type: 'pdf', size: '2.4 MB' },
  { id: 'r2', title: 'Dataset: Sales Analytics', type: 'code', size: '1.1 MB' },
  { id: 'r3', title: 'Pandas Documentation', type: 'link', size: '' },
  { id: 'r4', title: 'Matplotlib Gallery', type: 'link', size: '' },
]
...
export default function CoursePage() {
  ...
  function sendMessage() {
    ...
    // TODO: POST /api/ai-chat { messages, module_context_id, session_id }
    setTimeout(() => {
      setMessages((m) => [...m, {
        role: 'assistant',
        content: `Great question about "${userMsg}"! In EDA, we typically start by examining distributions, handling missing values, and identifying outliers. Would you like me to explain any specific aspect in more detail?`,
      }])
    }, 900)
  }
  ...
  function handleQuizSubmit() {
    ...
    const correct = quiz.questions.filter((q, i) => quizAnswers[q.id] === q.correct_index).length
    // TODO: POST /api/quiz/submit { quiz_id, answers, student_id }
    showToast(`Quiz complete! You scored ${correct}/${quiz.questions.length} 🎯`, correct >= 2 ? 'success' : 'warning')
  }
  ...
  const weekGroups = course.modules.reduce<Record<number, CourseModule[]>>((acc, m) => {
      ...
      <div className="flex h-[calc(100vh-64px)]">
        ...
        <aside className="hidden lg:flex flex-col w-[300px] bg-white border-r border-brand-border overflow-y-auto flex-shrink-0">
          <div className="p-5 border-b border-brand-border">
            <h2 className="font-sora text-sm font-bold text-brand-navy leading-snug">{course.title}</h2>
            <div className="flex items-center gap-2 mt-2">
              <div className="flex-1 bg-gray-100 rounded-full h-1.5">
                <div className="bg-brand-orange h-full rounded-full" style={{ width: `${course.progress}%` }} />
              </div>
              <span className="text-xs text-brand-muted font-dm-sans">{course.progress}%</span>
            </div>
          </div>
          ...
          <div className="flex-1 py-2">
            {Object.entries(weekGroups).map(([week, mods]) => {
              ...
              return (
                <div key={wk}>
                  <button
                    onClick={() => setExpandedWeek(isOpen ? -1 : wk)}
                    className="w-full flex items-center justify-between px-5 py-3 hover:bg-brand-bg transition text-left"
                  >
                    <span className="text-xs font-semibold text-brand-muted uppercase tracking-wider font-dm-sans">
                      Week {wk}
                    </span>
            ...
            <div className="flex gap-0">
              {tabs.map(({ key, label }) => (
                <button
                  ...
                  onClick={() => setTab(key)}
                  className={clsx(
                    'px-5 py-4 text-sm font-medium font-dm-sans border-b-2 transition-colors',
                    tab === key
          ...
          <div className="max-w-3xl mx-auto px-6 py-8">
            ...
            {tab === 'overview' && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                ...
                <div className="flex gap-3">
                  <button
                    onClick={() => { setCompleted(true); showToast('Module marked as complete! ✅', 'success'); setTab('quiz') }}
                    className={clsx(
                      'flex-1 py-3 rounded-xl font-semibold text-sm font-dm-sans transition-all flex items-center justify-center gap-2',
                      completed
                        ? 'bg-brand-success text-white cursor-default'
                        : 'bg-brand-orange text-white hover:bg-brand-orange-dark',
                    )}
                  >
                    <CheckCircle size={16} /> {completed ? 'Completed ✓' : 'Mark as Complete'}
                  </button>
                  <button
                    onClick={() => setChatOpen(true)}
                    className="flex items-center gap-2 border-2 border-brand-orange text-brand-orange px-5 py-3 rounded-xl font-semibold text-sm font-dm-sans hover:bg-brand-orange hover:text-white transition-all"
                  >
            ...
            {tab === 'resources' && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                  ...
                  {resources.map((r) => (
                    <div key={r.id} className="flex items-center gap-4 bg-white rounded-xl border border-brand-border p-4 hover:border-brand-orange/40 transition group">
                      <div className={clsx(
                        ...
                        r.type === 'pdf' ? 'bg-red-50' : r.type === 'code' ? 'bg-blue-50' : 'bg-purple-50',
                      )}>
                        {r.type === 'pdf' ? <FileText size={18} className="text-red-500" />
                          : r.type === 'code' ? <Code2 size={18} className="text-blue-500" />
                          : <Link2 size={18} className="text-purple-500" />}
                      </div>
                      <div className="flex-1">
                        <p className="text-sm font-semibold text-brand-navy font-dm-sans">{r.title}</p>
                        {r.size && <p className="text-xs text-brand-muted font-dm-sans mt-0.5">{r.size}</p>}
                      </div>
                      <button
                        onClick={() => showToast(`Downloading "${r.title}"…`, 'info')}
                ...
                {/* TODO: GET /api/modules/:id/resources */}
              </motion.div>
            )}
            ...
            {tab === 'discussion' && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                ...
                <div className="text-center py-12">
                  ...
                  {/* TODO: GET /api/discussions/:module_id */}
                </div>
              </motion.div>
            )}
          </div>
          ...
          {chatOpen && (
            ...
            >
              ...
              <div className="h-72 overflow-y-auto p-4 space-y-3 bg-brand-bg">
                {messages.map((msg, i) => (
                  <div key={i} className={clsx('flex', msg.role === 'user' ? 'justify-end' : 'justify-start')}>
                    <div className={clsx('max-w-[85%] text-xs leading-relaxed px-3.5 py-2.5',
              ...
              <div className="flex gap-2 p-3 border-t border-brand-border bg-white">
                ...
                <button
                  onClick={sendMessage}
                  className="w-8 h-8 bg-brand-orange rounded-lg flex items-center justify-center hover:bg-brand-orange-dark transition"
                >
  ...
  return (
    <div className="space-y-5">
      <div className={clsx(
        ...
        pct >= 70 ? 'bg-green-50 border-brand-success' : 'bg-orange-50 border-brand-warning',
      )}>
        <div className="text-4xl font-sora font-bold mb-1">{pct}%</div>
        <p className="font-dm-sans text-sm">
          {correct} of {questions.length} correct · {pct >= 70 ? '🎉 Passed!' : '📚 Keep studying'}
        </p>
      </div>
      ...
      {questions.map((q, qi) => {
        ...
        return (
          <div key={q.id} className={clsx(
            ...
            isCorrect ? 'border-brand-success/40' : 'border-brand-danger/40',
          )}>
            <p className="font-sora text-sm font-bold text-brand-navy mb-3">
              <span className={clsx('mr-2', isCorrect ? 'text-brand-success' : 'text-brand-danger')}>
                Q{qi + 1}.
              </span>
              {q.body}
            </p>
            <div className="space-y-2">
              {q.options.map((opt, oi) => (
                <div key={oi} className={clsx(
                  'px-3 py-2 rounded-lg text-xs font-dm-sans',
                  oi === q.correct_index ? 'bg-green-100 text-green-700 font-semibold' :
                  oi === userAns && !isCorrect ? 'bg-red-100 text-red-700' : 'bg-gray-50 text-gray-500',
                )}>
      ...
      <button onClick={onReset} className="w-full border-2 border-brand-border text-brand-navy py-3 rounded-xl font-semibold text-sm font-dm-sans hover:border-brand-orange/40 hover:text-brand-orange transition">
        Retake Quiz
      </button>
    </div>
  )
